...
def _train_cmrp_variant(
    ...
    for epoch in range(int(config["training"]["epochs"])):
        ...
        val_result = run_cmrp_eval_epoch(model, val_loader, device, variant=variant)
        key = _selection_key(
        ...
        history.append(
            {
                "epoch": epoch + 1,
                **{f"train_{name}": value for name, value in train_result.items()},
                "val_loss": val_result["loss"],
                "val_auc": val_result["metrics"].get("auc"),
                "val_balanced_accuracy": val_result["metrics"].get("balanced_accuracy"),
            }
        )
        print(
...
def run(
    ...
    *,
    ...
    if any(item in CMRP_VARIANTS for item in requested) and target is not None:
        cmrp_cfg = config.get("cmrp_uda") or {}
    ...
    target_adapt_set = (
        None
        if not cmrp_target_split or target is None
