...
```text
git fetch origin
git checkout codex/scan-filtered-loader
git pull --ff-only
...
当前仓库提供的 `journal_dual_shift_scan_filtered_1p5t_mci_ad.yaml` 是
`MCI_vs_AD` 模板。执行 `CN_vs_AD` 时复制为任务专用 YAML，并只修改以下字段：
...
```text
<PYTHON> experiments/train_journal.py --config_path <SMOKE_CONFIG>.yaml --direction ADNI_to_NACC --variants ce_x mixstyle_x apic_v3_2_x --output-dir <OUTPUT_ROOT>/<task>/smoke/adni_to_nacc --device cpu
```
...
```text
<PYTHON> experiments/train_journal.py --config_path <TASK_CONFIG>.yaml --study --directions ADNI_to_NACC --seeds 42 43 --output-dir <OUTPUT_ROOT>/<task>/adni_to_nacc
```
...
1. checkout 分支 codex/scan-filtered-loader，并确认 commit 与主控提供的 commit 一致。
...
8. 任务只能是 `MCI_vs_AD`（mapping `{2:0,3:1}`）或 `CN_vs_AD`（mapping `{1:0,3:1}`）；方向是 `<DIRECTION>`；seed 为 42、43；variants 为 ce_x、mixstyle_x、apic_v3_2_x。
